...
"""This script can be used to format code using 'clang-format'.

Usage:
        python utilities/indent.py
...
(see  deal.ii-qc/cmake/git-hooks/pre-commit.in).
"""
...
def parse_arguments():
    ...
    parser.add_argument("-dirs", "--directories",
                        default="include,source,tests",
                        help="Comma-delimited list of directories to work on."
                        "By default only \"include\", \"source\" and \"tests\" "
...
def check_clang_format_version(clang_format_binary, compatible_version_list):
    ...
    if clang_format_binary:
        ...
        sys.exit(
            ...
            ***
            ***   You can run
            ***       'utilities/download_clang_format'
            ***   or
            ***       'utilities/compile_clang_format'
...
if __name__ == "__main__":
    ...
    # utlitlies/programs/clang-6/bin
    #
    if not PARSED_ARGUMENTS.clang_format_binary:
        os.environ["PATH"] += ':' + \
            os.getcwd() + "/utilities/programs/clang-6/bin"
