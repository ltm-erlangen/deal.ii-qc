...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::run(
  const bool relaxed_configuration_as_reference)
{
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::output_results(
  const double       time,
  const unsigned int timestep_no) const
{
  ...
  const std::string atom_data_filename = dealiiqc::Utilities::data_out_filename(
    atom_data_name, timestep_no, this_mpi_process, ".vtp");
  ...
  if (this_mpi_process == 0)
    {
      ...
      for (unsigned int i = 0; i < n_mpi_processes; ++i)
        atom_data_filenames.push_back(dealiiqc::Utilities::data_out_filename(
          atom_data_name, timestep_no, i, ".vtp"));

      const std::string pvtp_atom_data_master_filename =
        (atom_data_name + dealii::Utilities::int_to_string(timestep_no, 4) +
         ".pvtp");

      std::ofstream pvtp_atom_data_master(
      ...
      times_and_atom_data_names.push_back(
        std::make_pair(time, pvtp_atom_data_master_filename.c_str()));
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::setup_triangulation()
{
  ...
  refinement_function.initialize(
  ...
  const double refinement_parameter =
    initial_refinement_params.refinement_parameter;
  ...
  for (unsigned int cycle = 0; cycle < n_refinement_cycles; cycle++)
    {
      Vector<float> blind_error_estimate_per_cell(
      ...
      GridRefinement::refine_and_coarsen_fixed_number(
        triangulation, blind_error_estimate_per_cell, refinement_parameter, 0);
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::setup_cell_molecules()
{
...
QC<dim, PotentialType, atomicity>::setup_cell_energy_molecules()
{
  ...
  cluster_weights_method =
    configure_qc.get_cluster_weights<dim, atomicity, spacedim>();
...
template <typename T>
void
QC<dim, PotentialType, atomicity>::write_mesh(T &os, const std::string &type)
{
  GridOut grid_out;
  if (!type.compare("eps"))
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::initialize_boundary_functions()
{
  ...
  for (auto &single_bc : boundary_ids_to_function_expressions)
    {
      ...
      dirichlet_boundary_functions.insert(std::make_pair(
        single_bc.first,
        std::make_pair(
          component_mask,
          std::make_shared<FunctionParser<spacedim>>(dim * atomicity, 0.))));

      dirichlet_boundary_functions[single_bc.first].second->initialize(
        (spacedim == 3) ? "x,y,z,t" : (spacedim == 2 ? "x,y,t" : "x,t"),
        single_bc.second,
        typename FunctionParser<spacedim>::ConstMap(),
        true);
    }
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::setup_boundary_conditions(const double time)
{
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::initialize_external_potential_fields(
  const double initial_time)
{
  for (const auto &entry : configure_qc.get_external_potential_fields())
    {
      auto external_potential_field_iterator = external_potential_fields.insert(
        std::make_pair(entry.first.first,
                       std::make_shared<PotentialFieldFunctionParser<spacedim>>(
                         entry.first.second, initial_time)));
...
QC<dim, PotentialType, atomicity>::setup_system()
{
  ...
  Assert(atomicity > 1 ? std::equal(n_dofs_per_block.begin() +
                                      1, // Check from the second element.
...

  {
    ...
    for (int atom_stamp = 0; atom_stamp < atomicity; ++atom_stamp)
      {
        locally_owned_partitioning[atom_stamp] =
          locally_owned_set.get_view(n_dofs_per_block[0] * (atom_stamp),
                                     n_dofs_per_block[0] * (atom_stamp + 1));
        locally_relevant_partitioning[atom_stamp] =
          locally_relevant_set.get_view(n_dofs_per_block[0] * (atom_stamp),
                                        n_dofs_per_block[0] * (atom_stamp + 1));
      }
  }
  ...
  distributed_displacement.reinit(locally_owned_partitioning, mpi_communicator);
  ...
  if (std::dynamic_pointer_cast<Cluster::WeightsByOptimalSummationRules<dim>>(
    ...
    cluster_weights_method->compute_dof_inverse_masses(
      inverse_masses,
      cell_molecule_data,
      dof_handler,
      hanging_node_constraints);
  ...
  // TODO: use TriaAccessor<>::set_user_pointer() to associate AssemblyData with
  // a cell
...
QC<dim, PotentialType, atomicity>::setup_fe_values_objects()
{
  ...
  for (types::DoFCellIteratorType<dim, spacedim> cell =
       ...
       cell++)
    {
      ...
      for (unsigned int q = 0; q < n_energy_molecules_in_cell;
           ++q, ++cell_energy_molecule_iterator)
        {
          // const_iter->second yields the actual atom
          for (int d = 0; d < dim; ++d)
            // Copying only dim-dimensions.
            points[q][d] = cell_energy_molecule_iterator->second
                             .position_inside_reference_cell[d];

          weights_per_atom[q] =
            cell_energy_molecule_iterator->second.cluster_weight;
          cell_energy_molecule_iterator->second.local_index = q;
        }

      Assert(cell_energy_molecule_iterator ==
               energy_molecules_range.first.second,
      ...
      Assert(data.fe_values.use_count() == 0, ExcInternalError());

      // Now we are ready to initialize FEValues object.
      data.fe_values = std::make_shared<FEValues<dim, spacedim>>(
        mapping, fe, Quadrature<dim>(points, weights_per_atom), update_values);
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::update_positions()
{
  ...
  for (auto cell = dof_handler.begin_active(); cell != dof_handler.end();
       cell++)
    {
      ...
      Assert(it != cells_to_data.end(), ExcInternalError());

      auto &data = it->second;

      std::pair<types::CellMoleculeIteratorType<dim, atomicity, spacedim>,
                types::CellMoleculeIteratorType<dim, atomicity, spacedim>>
        cell_energy_molecules_range =
          cell_molecule_data.cell_energy_molecules.equal_range(cell);

#ifdef DEBUG
      const unsigned int n_energy_molecules_in_cell =
        cell_molecule_data.cell_energy_molecules.count(cell);
      for (int atom_stamp = 0; atom_stamp < atomicity; ++atom_stamp)
        // Size of the displacements should be same as the total number of the
        // energy molecules on a per cell basis.
        Assert(data.displacements[atom_stamp].size() ==
                 n_energy_molecules_in_cell,
               ExcInternalError());
#endif

      // If this locally relevant active cell doesn't contain any atoms
      // continue active cell loop
      if (cell_energy_molecules_range.first ==
          cell_energy_molecules_range.second)
        continue;

      for (int atom_stamp = 0; atom_stamp < atomicity; ++atom_stamp)
        // Get displacement field for each atom_stamp at all quadrature points
        // of this cell.
        (*data.fe_values)[u_fe[atom_stamp]].get_function_values(
          locally_relevant_displacement, data.displacements[atom_stamp]);
      const auto &displacements = data.displacements;

      // Update energy molecules positions.
      for (unsigned int i = 0; cell_energy_molecules_range.first !=
                               cell_energy_molecules_range.second;
           cell_energy_molecules_range.first++, i++)
        for (int atom_stamp = 0; atom_stamp < atomicity; ++atom_stamp)
          for (int d = 0; d < dim; ++d)
            // Copy only dim-dimensions.
            cell_energy_molecules_range.first->second.atoms[atom_stamp]
              .position[d] =
              cell_energy_molecules_range.first->second.atoms[atom_stamp]
                .initial_position[d] +
      ...
      Assert(cell_energy_molecules_range.first ==
               cell_energy_molecules_range.second,
             ExcInternalError());
    }
}
...
QC<dim, PotentialType, atomicity>::update_neighbor_lists()
{
  ...
  // if( (iter_count % neigh_modify_delay)==0 || (max_abs_displacement >
  // neigh_skin)   )
  //   update_neighbour_lists();
  neighbor_lists = molecule_handler.get_neighbor_lists(
    cell_molecule_data.cell_energy_molecules);
}
...
double
QC<dim, PotentialType, atomicity>::compute(vector_t &gradient) const
{
  ...
  const double energy_per_process =
    neighbor_lists.empty() ? 0. : compute_local<ComputeGradient>(gradient);
...
template <bool ComputeGradient>
double
QC<dim, PotentialType, atomicity>::compute_local(vector_t &gradient) const
{
  ...
  const unsigned int                           dofs_per_cell = fe.dofs_per_cell;
  std::vector<dealii::types::global_dof_index> local_dof_indices_I(
    dofs_per_cell),
    local_dof_indices_J(dofs_per_cell);
  Vector<double> local_gradient_I(dofs_per_cell),
    local_gradient_J(dofs_per_cell);
  ...
  const types::DoFCellIteratorType<dim, spacedim> dof_cell_I_first(
    &triangulation, cell_I_first->level(), cell_I_first->index(), &dof_handler);

  const types::DoFCellIteratorType<dim, spacedim> dof_cell_J_first(
    &triangulation, cell_J_first->level(), cell_J_first->index(), &dof_handler);

  // Locate the first pair of cells I-J.
  typename std::map<types::DoFCellIteratorType<dim, spacedim>,
                    AssemblyData>::const_iterator cell_data_I =
                                                    cells_to_data.find(
                                                      dof_cell_I_first),
                                                  cell_data_J =
                                                    cells_to_data.find(
  ...
  for (const auto &cell_pair_cell_molecule_pair : neighbor_lists)
    {
      ...
      const double scale_energy =
        (optimal_summation_rule_used && molecule_J.cluster_weight == 0) ?
          molecule_I.cluster_weight :
          0.5 * (molecule_I.cluster_weight + molecule_J.cluster_weight);
      ...
      if (ComputeGradient)
        {
          const types::DoFCellIteratorType<dim, spacedim> dof_cell_I(
            &triangulation, cell_I->level(), cell_I->index(), &dof_handler);

          const types::DoFCellIteratorType<dim, spacedim> dof_cell_J(
            &triangulation, cell_J->level(), cell_J->index(), &dof_handler);
          ...
          const unsigned int qI = molecule_I.local_index,
          ...
          for (unsigned int k = 0; k < dofs_per_cell; ++k)
            {
              ...
              local_gradient_J[k] +=
                gradient_J[atom_stamp][nonzero_comp] *
...
template <int dim, typename PotentialType, int atomicity>
void
QC<dim, PotentialType, atomicity>::minimize_energy(const double time)
{
  if (time >= 0.)
    {
      ...
      if (configure_qc.get_minimizer_name() == "FIRE")
        {
          ...
          typename SolverFIRE<vector_t>::AdditionalData additional_data_fire(
            fire_parameters.initial_time_step,
...
#define SINGLE_QC_INSTANTIATION(_DIM, _POTENTIAL, _ATOMICITY)             \
  ...
  template double QC<_DIM, _POTENTIAL, _ATOMICITY>::compute<true>(        \
    dealii::TrilinosWrappers::MPI::BlockVector &) const;                  \
  template double QC<_DIM, _POTENTIAL, _ATOMICITY>::compute<false>(       \
    dealii::TrilinosWrappers::MPI::BlockVector &) const;                  \
  template double QC<_DIM, _POTENTIAL, _ATOMICITY>::compute_local<true>(  \
    dealii::TrilinosWrappers::MPI::BlockVector &) const;                  \
