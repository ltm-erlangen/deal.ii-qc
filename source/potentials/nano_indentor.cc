...
namespace
{
  template <int dim>
  Tensor<1, dim>
  get_normalized_tensor(const Tensor<1, dim> &in_tensor)
  {
...
template <int dim>
NanoIndentor<dim>::NanoIndentor(const Point<dim> &    initial_location,
...
template <int dim>
void
NanoIndentor<dim>::initialize(const std::string &                  variables,
...
template <int dim>
void
NanoIndentor<dim>::set_time(const double new_time)
{
  FunctionTime<double>::set_time(new_time);
  indentor_position_function.set_time(new_time);
  current_location =
    initial_location +
    indentor_position_function.value(Point<1>(new_time)) * direction;
}



#define NANO_INDENTOR(R, X, _SPACE_DIM) template class NanoIndentor<_SPACE_DIM>;
