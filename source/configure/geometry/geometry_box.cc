...
namespace Geometry
{
    ...
    dealii::parallel::shared::Triangulation<dim> &mesh) const
  {
    ...
    GridGenerator::subdivided_hyper_rectangle(
      mesh, rep_vec, bottom_left, top_right, true);
  ...
  template <>
  void
  Box<1>::parse_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("Geometry");
    {
      Base<1>::n_initial_global_refinements =
  ...
  template <>
  void
  Box<2>::parse_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("Geometry");
    {
      Base<2>::n_initial_global_refinements =
        prm.get_integer("Number of initial global refinements");
      prm.enter_subsection("Box");
      {
        center[0]      = prm.get_double("X center");
  ...
  template <>
  void
  Box<3>::parse_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("Geometry");
    {
      Base<3>::n_initial_global_refinements =
        prm.get_integer("Number of initial global refinements");
      prm.enter_subsection("Box");
      {
        center[0]      = prm.get_double("X center");
  ...
  Box<dim>::declare_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("Geometry");
    {
      prm.enter_subsection("Box");
      {
        prm.declare_entry("X extent",
                          "10",
                          Patterns::Double(0),
                          "Extent of the box in x-direction. Units: Angstrom.");
        prm.declare_entry("Y extent",
                          "10",
                          Patterns::Double(0),
