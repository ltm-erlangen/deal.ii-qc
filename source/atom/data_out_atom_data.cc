...
namespace DataOutBase
{
  ...
  {
  public:
    VtpStream(std::ostream &stream, const VtkFlags &flags)
      : stream(stream)
      , flags(flags)
    {}
    ...
    template <int dim>
    void
    write_point(const Point<dim> &p);

    void
    write_scalar(const double);
...
namespace
{
   ...
   */
  void
  write_vtp_header(std::ostream &out, const DataOutBase::VtkFlags &flags)
  {
    ...
    if (flags.print_date_and_time == true)
      out << "#This file was generated by the deal.II-qc library"
          << " on " << dealii::Utilities::System::get_time() << " at "
   ...
   */
  void
  write_vtp_footer(std::ostream &out)
  {
    AssertThrow(out, ExcIO());
    out << " </PolyData>\n";
    out << "</VTKFile>\n";
  ...
  write_vtp_main(
    ...
    std::ostream &               out)
  {
    AssertThrow(cell_molecules.size() > 0, ExcMessage("No atom data to write"));
...
void
DataOutAtomData::write_pvtp_record(
  const std::vector<std::string> &     vtp_file_names,
  const dealii::DataOutBase::VtkFlags &flags,
  std::ostream &                       out)
{
  AssertThrow(out, ExcIO());
