...
MoleculeHandler<dim, atomicity, spacedim>::MoleculeHandler(
...
MoleculeHandler<dim, atomicity, spacedim>::get_neighbor_lists(
    ...
    &cell_energy_molecules) const
{
  ...
  Assert(cell_energy_molecules.size(), ExcInternalError());
  ...
  for (types::CellMoleculeConstIteratorType<dim, atomicity, spacedim> unique_I =
         cell_energy_molecules.cbegin();
    ...
    if (unique_I->first->is_locally_owned())
      {
        ...
        for (types::CellMoleculeConstIteratorType<dim, atomicity, spacedim>
          ...
          {
            types::ConstCellIteratorType<dim, spacedim> cell_J =
              unique_J->first;
  ...
  for (const auto &cell_pair_IJ : cell_neighbor_lists)
    {
      ...
      std::pair<types::CellMoleculeConstIteratorType<dim, atomicity, spacedim>,
...
#define SINGLE_MOLECULE_HANDLER_INSTANTIATION(_DIM, _ATOMICITY, _SPACE_DIM) \
  template class MoleculeHandler<_DIM, _ATOMICITY, _SPACE_DIM>;
