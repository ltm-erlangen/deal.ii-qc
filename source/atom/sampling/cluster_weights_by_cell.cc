...
namespace Cluster
{
  ...
  WeightsByCell<dim, atomicity, spacedim>::WeightsByCell(
  ...
  WeightsByCell<dim, atomicity, spacedim>::update_cluster_weights(
    ...
    for (auto &energy_molecule : cell_energy_molecules)
      {
        ...
        energy_molecule.second.cluster_weight *=
          ...
          static_cast<double>(
            n_cluster_molecules_per_cell.at(energy_molecule.first));
      }

      //---Check in Debug mode that n_molecules and n_energy_molecules computed
      // here
    ...
    for (types::CellIteratorType<dim, spacedim> cell =
         ...
         cell++)
      {
        ...
        const auto n_cluster_molecules_in_cell =
          CellMoleculeTools::n_cluster_molecules_in_cell(cell,
                                                         cell_energy_molecules);

        Assert(n_molecules_per_cell[cell] == molecules_range.second,
               ExcInternalError());

        Assert(n_cluster_molecules_per_cell[cell] ==
