...
namespace Cluster
{
        ...
        &cell_molecules) const
  {
    const double molecule_density =
      CellMoleculeTools::compute_molecule_density<dim, atomicity, spacedim>(
        triangulation, cell_molecules);
    ...
    const double full_weight =
      2. * dealiiqc::Utilities::hyperball_segment_volume<dim>(rep_distance, 0);
        ...
        else if (cell->level() < n_levels - 1)
          {
            for (const auto &index : this_cell_sampling_indices)
              if (!weight_assigned[index] && !(this->is_quadrature_type(index)))
                {
                  weight[index] -=
                    dealiiqc::Utilities::hyperball_segment_volume<dim>(
                      rep_distance, half_cell_width) /
                  ...
                  // rely on suitable adjustment to the weight of the
                  // inner-element sampling point.
              ...
              if (this->is_quadrature_type(index))
                {
                  const double inner_weight =
                    molecule_density * cell->measure() - vertex_weights_sum;
    ...
    std::vector<unsigned int> this_cell_sampling_indices =
      this->get_sampling_indices(unique_cell);

    std::vector<Point<spacedim>> this_cell_sampling_points =
      this->get_sampling_points(unique_cell);
    ...
    for (auto &cell_molecule : cell_molecules)
      {
        ...
        if (squared_distance < squared_energy_radius)
          {
            // Squared distance to the closest sampling point is less than
            // THICK_EPS
...
#define SINGLE_WEIGHTS_BY_OPTIMAL_INSTANTIATION(_DIM, _ATOMICITY, _SPACE_DIM) \
  template class WeightsByOptimalSummationRules<_DIM, _ATOMICITY, _SPACE_DIM>;
