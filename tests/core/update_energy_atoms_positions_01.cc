...
Problem<dim, PotentialType>::run()
{
  ...
  QC<dim, PotentialType>::locally_relevant_displacement = u;
  QC<dim, PotentialType>::update_positions();
  const double energy_1 = QC<dim, PotentialType>::template compute(
    QC<dim, PotentialType>::locally_relevant_gradient);
  ...
  QC<dim, PotentialType>::locally_relevant_displacement = u;
  QC<dim, PotentialType>::update_positions();
  const double energy_2 = QC<dim, PotentialType>::template compute(
    QC<dim, PotentialType>::locally_relevant_gradient);

  AssertThrow(energy_0 == energy_2, ExcInternalError());
