...
// The two molecules are far appart that the total energy is just the
// intra molecular interaction.
// This test is a clone of energy_and_gradient_lj_01.
...
template <int dim, typename PotentialType, int atomicity>
void
Problem<dim, PotentialType, atomicity>::partial_run (const double &blessed_energy,
                                                     const double &blessed_gradient)
{
  ...
  const double energy =
    QC<dim, PotentialType, atomicity>::template
    compute<true> (this->locally_relevant_gradient);
