...
test(const double &r, const double &cutoff_radius)
{
  ...
  atom_1.type     = 0;
  atom_1.position = (spacedim == 3) ?
                      Point<spacedim>(1.12 + r, 2.4, 7.3) :
                      (spacedim == 2 ? Point<spacedim>(1.12 + r, 2.4) :
                                       Point<spacedim>(1.12 + r));

  atom_2.type = 1;
  atom_2.position =
    (spacedim == 3) ?
      Point<spacedim>(1.12, 2.4, 7.3) :
  ...
  const Tensor<1, spacedim> lj_gradient_0 =
    rij * lj_energy_gradient_0.second / rij.norm();
  ...
  const Tensor<1, spacedim> lj_gradient_1 =
    -rij * lj_energy_gradient_1.second / rij.norm();
  ...
  const std::pair<double, Tensor<1, spacedim>> enegry_and_gradient_0 =
    ComputeTools::energy_and_gradient(lj, atom_1, atom_2);
  const std::pair<double, Tensor<1, spacedim>> enegry_and_gradient_1 =
    ComputeTools::energy_and_gradient(lj, atom_2, atom_1);
