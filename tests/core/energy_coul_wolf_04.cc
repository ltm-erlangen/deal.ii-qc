...
Problem<dim, PotentialType>::partial_run(const double &blessed_energy)
{
  ...
  unsigned int n_mpi_processes(dealii::Utilities::MPI::n_mpi_processes(
    QC<dim, PotentialType>::mpi_communicator)),
    this_mpi_process(dealii::Utilities::MPI::this_mpi_process(
      QC<dim, PotentialType>::mpi_communicator));
      ...
      if (p == this_mpi_process)
        {
          for (auto entry : QC<dim, PotentialType>::neighbor_lists)
            std::cout << "Atom I: "
                      ...
                      << " Cluster weight: "
                      << entry.second.second->second.cluster_weight
                      << std::endl;
        }
      MPI_Barrier(QC<dim, PotentialType>::mpi_communicator);
    }
