...
void
test(const double eps, const double rm, const double cutoff_radius)
{
  const std::vector<double> lj_params = {eps, rm};

  Potential::PairLJCutManager lj(cutoff_radius);
  lj.declare_interactions(0, 0, Potential::InteractionTypes::LJ, lj_params);
  ...
  for (auto &atom : molecule.atoms)
    {
      for (int sd = 0; sd < spacedim; ++sd)
        atom.position[sd] = static_cast<double>(Testing::rand()) / RAND_MAX;
      atom.type = 0;
    }
  ...
  std::cout << "Energy: " << intra_molecular.first << std::endl
            << "Gradient: " << std::endl;

  for (const auto &gradient_tensor : intra_molecular.second)
    std::cout << "\t\t" << gradient_tensor << std::endl;
  ...
  std::cout << "External force on molecule: " << -external_forces << std::endl;

  for (int d = 0; d < spacedim; d++)
    AssertThrow(std::fabs(external_forces[d]) < 1e-12, ExcInternalError());
}
