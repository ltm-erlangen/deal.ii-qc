...
Problem<dim, PotentialType>::partial_run()
{
  ...
  for (unsigned int i = 0; i < locally_owned_dofs.n_elements(); ++i)
    if (QC<dim, PotentialType>::locally_relevant_gradient(
          locally_owned_dofs.nth_index_in_set(i)) == 0.)
      n_zeros++;
...
/*
 ...
 print("Energy : ");
 float(at(shifted_energy(p,q,r,rc,alpha),
 [p=1.,q=-1.,r=sqrt(2),rc=8.25,alpha=0.25]));
