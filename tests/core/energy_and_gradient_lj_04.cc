...
// o--x--x-----o
//
// 12 entries of the gradient of the total energy are zeros.
      ...
      ConfigureQC(std::make_shared<std::istringstream>(s.c_str())))
{
  ...
  auto &triangulation = QC<dim, PotentialType>::triangulation;
...
Problem<dim, PotentialType>::partial_run()
{
  ...
  for (auto entry = gradient.block(0).begin(); entry != gradient.block(0).end();
       ++entry)
    if (*entry == 0.)
      n_zeros++;
  ...
  n_zeros =
    dealii::Utilities::MPI::sum(n_zeros,
  ...
  QC<dim, PotentialType>::pcout
    << "l1 norm      = "
    << QC<dim, PotentialType>::locally_relevant_gradient.l1_norm() << std::endl
    << "l2 norm      = "
    << QC<dim, PotentialType>::locally_relevant_gradient.l2_norm() << std::endl
    << "linfty norm  = "
    << QC<dim, PotentialType>::locally_relevant_gradient.linfty_norm()
    << std::endl;

  QC<dim, PotentialType>::pcout << "n_dofs       = " << n_dofs << std::endl;

  QC<dim, PotentialType>::pcout << "n_grad_zeros = " << n_zeros << std::endl;
  ...
  if (dealii::Utilities::MPI::n_mpi_processes(
    ...
    for (unsigned int i = 0; i < n_dofs; i += dim)
      {
        for (int d = 0; d < dim; ++d)
          QC<dim, PotentialType>::pcout << gradient.block(0)[i + d] << "\t";
        QC<dim, PotentialType>::pcout << std::endl;
      }

#ifdef WRITE_GRID
  if (dealii::Utilities::MPI::this_mpi_process(
        QC<dim, PotentialType>::mpi_communicator) == 0)
    {
  ...
  try
    {
      ...
      oss << "set Dimension = " << dim << std::endl
          ...
          << "Atoms #" << std::endl
          << std::endl
          << "1 1 1  1.0 0.615  0.185 0." << std::endl
          << "2 2 1  1.0 0.730  0.730 0." << std::endl;
...
N10(x,y):=       if y < 0.5             then 0
            else if x > 1.0             then 0
            else if x < 0.5 and y > 0.5 then 4*(   x)*(y-0.5)
            else if x > 0.5 and y > 0.5 then 4*(1.-x)*(y-0.5);

N(x,y) := [N0(x,y) + 0.5*N8(x,y),
           N2(x,y) + 0.5*N8(x,y),
           N5(x,y),
           N7(x,y),
           N8(x,y),
