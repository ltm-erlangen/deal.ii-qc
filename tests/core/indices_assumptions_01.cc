...
template <int dim, typename PotentialType, int atomicity>
void Problem<dim, PotentialType, atomicity>::test ()
{
  AssertThrow (this->triangulation.n_active_cells()==1,
               ExcNotImplemented());

  const unsigned int n_dofs = this->dof_handler.n_dofs();

  std::cout << " * | Local DoF | Gloabl DoF | Component | Block (atom stamp) | Non-zero Component |" << std::endl
            << " * | :-------: | :--------: | :-------: | :----------------: | :----------------: |" << std::endl;

  std::vector<dealii::types::global_dof_index>
  local_to_global_dof_indices (n_dofs);

  this->dof_handler.begin_active()->get_dof_indices(local_to_global_dof_indices);

  // Loop over all elements in dof handler.
  for (unsigned int i = 0; i < n_dofs; ++i)
    {
      const unsigned int component    = this->fe.system_to_component_index(i).first;
      ...
      std::cout << std::setfill(' ')
                ...
                << " | " << std::setw(9) << component
                << " | " << std::setw(18) << atom_stamp
  ...
  if (true)
    {
      ...
      DoFTools::map_dofs_to_support_points(mapping, this->dof_handler, support_points);
