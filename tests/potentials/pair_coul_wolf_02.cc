     ...
     std::shared_ptr<Potential::PairCoulWolfManager> coulf_wolf_ptr)
{
  ...
  AssertThrow(fabs(energy_force_0.first - blessed_energy) <
                100. * std::numeric_limits<double>::epsilon(),
              ExcInternalError());
  AssertThrow(fabs(energy_force_0.second - blessed_force) <
                100. * std::numeric_limits<double>::epsilon(),
              ExcInternalError());
  ...
  AssertThrow(fabs(energy_force_1.first - blessed_energy) <
                100. * std::numeric_limits<double>::epsilon(),
              ExcInternalError());
  AssertThrow(fabs(energy_force_1.second - blessed_force) <
                100. * std::numeric_limits<double>::epsilon(),
              ExcInternalError());
