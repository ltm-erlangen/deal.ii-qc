...
template <int dim>
void
test(const bool is_electric_field, const Point<dim> &p, const double q)
{
  PotentialFieldFunctionParser<dim> potential(is_electric_field, 0.);

  potential.initialize((dim == 3) ? "x,y,z,t" : (dim == 2 ? "x,y,t" : "x,t"),
                       (dim == 3) ? "x*x+y*y+z*z+t" :
  ...
  if (is_electric_field)
    ...
    {
      AssertThrow(potential.value(p, q) == p.norm_square(), ExcInternalError());
    }
}
