...
template <int dim>
void test (const double radius, const double d)
{
  const double volume = dealiiqc::Utilities::
                        hyperball_segment_volume<dim>(radius, d);

  AssertThrow (dim==2 || dim==3,
               ExcNotImplemented());

  const double
  computed_volume = (dim==2)
                    ?
  ...
  if (d==0)
    ...
    {
      AssertThrow (Testing::almost_equal(0., volume, 10),
                   ExcInternalError());
    }
