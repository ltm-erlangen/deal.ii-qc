...
test()
{
  ...
  Point<dim> a = (dim == 3) ?
                   Point<dim>(0.23, 0.37, 1.33) :
                   (dim == 2 ? Point<dim>(0.23, 0.37) : Point<dim>(0.23));

  Point<dim> b = (dim == 3) ?
                   Point<dim>(0.47, 0.63, 1.77) :
  ...
  Point<dim> p = (dim == 3) ?
                   Point<dim>(0.33, 0.54, 1.47) :
                   (dim == 2 ? Point<dim>(0.33, 0.54) : Point<dim>(0.33));

  const auto closest = dealiiqc::Utilities::find_closest_point(p, points_1);

  const auto closest_vertex =
    dealiiqc::Utilities::find_closest_vertex(p, tria.begin_active());

  AssertThrow(closest.second == closest_vertex.second, ExcInternalError());
