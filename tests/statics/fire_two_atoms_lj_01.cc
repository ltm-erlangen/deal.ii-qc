...
public:
  ...
  void
  statics(const double tol);
  double
  compute(vector_t &, const vector_t &);
};
...
template <int dim, typename PotentialType>
double
Problem<dim, PotentialType>::compute(vector_t &G, const vector_t &u)
{
...
template <int dim, typename PotentialType>
void
Problem<dim, PotentialType>::statics(const double tol)
{
  ...
  AssertThrow((n_iterations > 47) && (n_iterations < 51),
  ...
  QC<dim, PotentialType>::pcout
    ...
    << QC<dim, PotentialType>::template compute<false>(
         QC<dim, PotentialType>::locally_relevant_gradient)
    << "eV within 48 to 50 iterations." << std::endl;

  QC<dim, PotentialType>::pcout << "Final positions of the two atoms:"
                                << std::endl;

  for (const auto &entry :
       QC<dim, PotentialType>::cell_molecule_data.cell_energy_molecules)
    QC<dim, PotentialType>::pcout << entry.second.atoms[0].position
                                  << std::endl;
}
