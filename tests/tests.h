...
namespace Testing
{
  ...
  almost_equal(const T &x, const T &y, const unsigned int ulp)
  {
    // The machine epsilon has to be scaled to the magnitude of the values used
    // and multiplied by the desired precision in ULPs (units in the last place)
    return std::fabs(x - y) <= (std::numeric_limits<T>::epsilon() *
                                std::max(std::fabs(x), std::fabs(y)) * ulp) ||
  ...
  public:
      ...
      , n_processes(dealii::Utilities::MPI::n_mpi_processes(mpi_communicator))
    {
      std::string deallogname =
        "output" + dealii::Utilities::int_to_string(this_process);
   ...
   */
  void
  filter_out_xml_key(std::istream &     in,
