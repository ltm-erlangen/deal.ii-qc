...
public:
  ...
  run()
  {
    ...
    std::cout << "Total number of cell atoms: " << cell_molecules.size()
              << std::endl;

    // --- WeightsBySamplingPoints
    {
      Cluster::WeightsBySamplingPoints<dim> weights_by_sampling_points(
        config.get_cluster_radius(), config.get_maximum_cutoff_radius());

      weights_by_sampling_points.initialize(triangulation, QTrapez<dim>());
    ...
    //  --- WeightsByCell
    {
      Cluster::WeightsByCell<dim> weights_by_cell(
        config.get_cluster_radius(), config.get_maximum_cutoff_radius());

      weights_by_cell.initialize(triangulation, QTrapez<dim>());

      cell_molecule_data.cell_energy_molecules =
        weights_by_cell.update_cluster_weights(triangulation, cell_molecules);
