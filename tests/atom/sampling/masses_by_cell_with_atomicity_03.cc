...
public:
        ...
        ConfigureQC(std::make_shared<std::istringstream>(s.c_str())))
  {
    ...
    this->cell_molecule_data =
      CellMoleculeTools::build_cell_molecule_data<dim, atomicity>(
        *config.get_stream(),
        this->triangulation,
...
Problem<dim, PotentialType, atomicity>::partial_run()
{
  ...
  for (const auto &cell_molecule :
       this->cell_molecule_data.cell_energy_molecules)
    this->pcout << "Cell: " << cell_molecule.first << "\t"
                << "Molecule ref position inside cell: " << std::fixed
                << std::setprecision(1)
                << cell_molecule.second.position_inside_reference_cell << "\t"
                << "Cluster weight: " << cell_molecule.second.cluster_weight
                << std::endl;
