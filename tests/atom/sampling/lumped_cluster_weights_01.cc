...
// Compute the mass of the atomistic system of 2 atoms using lumped clustering
// approach.
...
public:
  ...
  void
  set_cluster_radius(const double &cluster_radius)
  {
...
public:
  ...
  void
  partial_run(const ConfigureQC &config);
};
...
Problem<dim, PotentialType>::partial_run(const ConfigureQC &config)
{
  ...
  std::cout << cluster_radius << "\t" << mass << "\t" << std::endl;
}
