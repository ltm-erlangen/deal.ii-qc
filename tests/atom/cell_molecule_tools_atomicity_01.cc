  ...
  run()
  {
    ...
    std::shared_ptr<Cluster::WeightsByBase<dim, atomicity, spacedim>>
      cluster_weights_method =
        config.get_cluster_weights<dim, atomicity, spacedim>();
    ...
    for (auto entry = cell_energy_molecules.begin();
         ...
         entry = cell_energy_molecules.upper_bound(entry->first))
      {
        const unsigned int n_molecules =
          CellMoleculeTools::molecules_range_in_cell<dim, atomicity, spacedim>(
            entry->first, cell_molecules)
            .second;

        const unsigned int n_energy_molecules =
          CellMoleculeTools::molecules_range_in_cell<dim, atomicity, spacedim>(
            entry->first, cell_energy_molecules)
            .second;

        std::cout << "Thrown molecules: " << n_molecules - n_energy_molecules
                  << std::endl;

        std::cout << "Energy molecules: " << cell_energy_molecules.size()
                  << std::endl
                  << std::endl;
        ...
        AssertThrow(cell_energy_molecules.count(entry->first) ==
                      n_energy_molecules,
                    ExcInternalError());
      }
  }
