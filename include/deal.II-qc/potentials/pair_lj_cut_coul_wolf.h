...
namespace Potential
{
  ...
  inline
  std::pair<double, double>
  PairLJCutCoulWolfManager::energy_and_gradient (const types::atom_type i_atom_type,
                                                 const types::atom_type j_atom_type,
                                                 const double &squared_distance) const
  {
    const std::pair<double, double>
    lj = lj_potential.
         energy_and_gradient<ComputeGradient> (i_atom_type,
                                               j_atom_type,
                                               squared_distance);
    const std::pair<double, double>
    coul = coul_wolf_potential.
           energy_and_gradient<ComputeGradient> (i_atom_type,
                                                 j_atom_type,
                                                 squared_distance);

    return std::make_pair (lj.first  + coul.first,
                           lj.second + coul.second);
  }
